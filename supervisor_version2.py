...
class Supervisor:
    ...
    def start_redis_server(self):
        ...
        if not os.path.exists(self.save_path_rdb):
            ...
            os.chmod(self.save_path_rdb, 0o777) # TODO SECURITY RISK check do we really need to have this permissions ?
    ...
    def stopgraph_saverdb_autoderiv(self):
        '''
        Stops the graph and plays any autorun derivatives
        '''
        # stop graph
        self.stop_graph()
        ...
        # Save rdb file
        self.r.save()
        self.full_save_path_rdb = os.path.join(self.save_path_rdb, self.rdb_filename)
        os.chmod(self.full_save_path_rdb, 0o777) # TODO SECURITY RISK check do we really need to have this permissions ?
        logger.info(f"RDB data saved to: {self.full_save_path_rdb }")

        save_path_nwb = os.path.join(self.save_path, 'NWB')

        try:
            included_derivatives = list(self.model['derivatives'])
            
            for derivative in included_derivatives:                
                deriv_params = self.model['derivatives'][derivative]
                if 'output_nwb_filename' in deriv_params['parameters']:
                    deriv_params['parameters']['output_nwb_filename'] = os.path.join(save_path_nwb, deriv_params['parameters']['output_nwb_filename'])
                
                if deriv_params['autorun'] == True:              
                    deriv_file_loc = deriv_params['module'] + '/' + deriv_params['name']
                    logger.info(f"Derivative starting")                
                    p_deri = subprocess.run(['python',
                                deriv_file_loc,
                                self.full_save_path_rdb,
                                self.host,
                                str(self.port),
                                json.dumps(deriv_params['parameters'])])                    
                    # p_deri.wait()
                    # if len(p_deri.stdout) > 0:
                    #     logger.debug(p_deri.stdout.decode())

                    if p_deri.returncode == 0:
                        logger.info(f"Derivative successfully run")
                    elif p_deri.returncode > 0:
                        raise DerivativeError(
                            f"{derivative} returned exit code {p_deri.returncode}.",
                            self.graph_file,
                            p_deri)
                    else:
                        logger.info(f"{derivative} was halted during execution with return code {p_nwb.returncode}, {signal.Signals(-p_nwb.returncode).name}")
        except KeyError:
            logger.info(f"No derivatives found")
    ...
    def print_derivatives(self):
        '''
        Prints model to supervisor
        '''
        # pdb.set_trace()
        logger.info(f'{self.model["derivatives"]}')
    ...
    def parseCommands(self, data):
        ...
        if cmd in ["loadgraph", "startgraph"]:
            ...
            self.stop_graph()
        elif cmd == "stopgraphsaverdbauto":
            logger.info("Stop graph, save  rdb, and run auto derivatives command received")
            self.stopgraph_saverdb_autoderiv()
        elif cmd == "saverdb":
            ...
            self.flush_db()
        elif cmd == "printderivatives":
            logger.info("Print Derivatives command received")
